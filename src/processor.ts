...
const verifyTilesetMetadata = (db: sqlite3.Database, id: string) => {
  ...
  if (inputTileset) {
    throw new Error(
      `この mbtiles は ${inputTileset} と同期していますが、 ${id} と同期しようとしているので、中断します。`
    );
  }

  const tileCountRow = db
    .prepare('SELECT count(*) AS "count" FROM tiles')
    .get() as { count: number } | undefined;
  if (tileCountRow?.count === 0) return; // OK to proceed
  throw new Error(
    `この mbtiles は既に他のタイルが入っているため、中断します。`
  );
};
...
const putTileImageData = (
  ...
  if (!_preparedInsertTileDataQuery) {
    _preparedInsertTileDataQuery = db.prepare(
...
const syncImagesTable = async (ctx: ProcessorCtx, um: MokurokuArray) => {
  const { db, gsiId, id, tileTransformer } = ctx;
  const totalCount = um.length;
  let insertCount = 0;
  let skipCount = 0;
  const queue = async.queue<MokurokuRow>(async (row) => {
    const exists = checkImageTile(db, row[3]);
    if (exists) {
      skipCount += 1;
      return;
    }
    let tileData = await getTileData(
      `https://cyberjapandata.gsi.go.jp/xyz/${gsiId}/${row[0]}`
    );
    if (tileTransformer) {
      tileData = await tileTransformer(tileData);
    }
    putTileImageData(db, row[3], parseInt(row[2], 10), tileData);
    insertCount += 1;
  }, 20);
  queue.push(um);
...
const syncTileRefTable = (ctx: ProcessorCtx, moku: MokurokuArray) => {
  ...
  for (const row of moku) {
    ...
    if (currentRow % 10_000 === 0) {
      console.timeLog(
        id,
        `[タイル同期] current=${currentRow} total=${mokuLen}`
      );
    }
  }
};
...
async function processor(
  ...
  if (mokuroku.status === "upToDate") {
    console.timeLog(
      id,
      "mbtiles が mokuroku と同期済みため、処理をスキップします。"
    );
    db.close();
  ...
  console.timeLog(
    id,
    `mokuroku に ${mokuroku.rows.length} 件のタイルが認識しました`
  );
  // md5でユニークをかける
  const uniqueMokuroku = mokurokuUniqueTiles(mokuroku.rows);
  console.timeLog(
    id,
    `mokuroku に ${uniqueMokuroku.length} 件のユニークなタイルを認識しました。`
  );
  ...
  if (!shutdownRequested) {
    ...
    writeMetadata(
      db,
      "attribution",
      '<a href="https://www.gsi.go.jp/" target="_blank">&copy; GSI Japan</a>'
    );
